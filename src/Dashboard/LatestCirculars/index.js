...
export function LatestCirculars() {
  ...
  const handleUpload = (e) => {
    ...
    dispatch(
      uploadFile(data,'circulars', token, (error) => {
        if (error) {
  ...
  const getList = (pageNo = 1) => {
    let token = state.token.jwtToken || getToken(); 
    dispatch(
      listFile(token,'circulars',pageNo, (error) => {
        if (error) {
          toast.error("Something went wrong");
        }
      })
  ...
  return (
    <Content title="circulars">
      <ToastContainer />
      <form className="input-group mb-3 search__form">
        <input type="text" className="form-control" onKeyUp={handleSearch} placeholder="Search Circulars" />
      ...
      <div className="row">
        {list && list.filesList && list.filesList.length ? list.filesList.map((item,index) => {
          return (
            <div key = {index} className="col-lg-3 col-md-4 col-sm-6 mb-30">
              <div className="card circulars__thumbnail">
        ...
        }):<div className="align-center p-50">No Circulars found</div>}
      </div>
